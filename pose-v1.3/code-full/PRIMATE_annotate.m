function ptsAll=PRIMATE_annotate(name)
...
% Liangfu Chen (liangfu.chen@nlpr.ia.ac.cn) 15/11/2015 revised
...
cols = {'r' 'g' 'b' 'c' 'm' 'y' 'k'};
trainfrs_pos = 1:2;%[16,17,34,39,52,100]; % training frames for positive

try
labeldata = load([name '/labels.mat']);
catch
...
Learn.Num_Parts = 14;
img_file_names = {};
Interest_Point.Scale=8;

%files=dir(name);imgcount=0;
img_file_names={};
for ii=1:length(trainfrs_pos)
%if isdir(files(ii).name),continue;end
img_file_names{ii}=[name sprintf('/im%04d.jpg',trainfrs_pos(ii))];
% imgcount=imgcount+1;
end
ptsAll=zeros([14,2,length(img_file_names)]);
%ptsAll=zeros([14,2,max(trainfrs_pos)]);
img_file_names
...
% img_file_names = genFileNames({Global.Image_Dir_Name},pos_image_ind, ...
%                               RUN_DIR,Global.Image_File_Name,Global.Image_Extension,Global.Num_Zeros);
...
if (length(Interest_Point.Scale)>1)
  error(['Multi-scale feature selection not currently implemented. ' ...
         'Set Interest_Point.Scale to be a single number not a vector.']);
end
...
end %%% end of loading label data files
...
if DEBUG
  ...
  for a=1:Learn.Num_Parts
    ...
    if (a>1) %% only show ellipses for non-landmark parts
      [ph,cy,L,l,th] = draw_ellipse([shape_mean_x(a);shape_mean_y(a)],...
                                    diag([shape_var_x(a);shape_var_y(a)]),cols{a},2,[]);
    end
  end
...
% save(fname,'shape_mean_x','shape_mean_y','shape_var_x','shape_var_y',...
%      'part_filter','hand_clicks_x','hand_clicks_y','part');

%%% copy conf_file into models directory too..
% config_fname = which(config_file);
% copyfile(config_fname,[RUN_DIR,'/',Global.Model_Dir_Name,'/',...
%                     Global.Config_File_Name,prefZeros(model_ind,Global.Num_Zeros),'.m']);
