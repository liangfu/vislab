function [im,opts,flog,plog,sclog] = superres_huber(W,Y,La,Lb,avim,alp,bet,opts)
%
% function [im,opts,flog,plog,sclog] = superres_huber(W,Y,La,Lb,avim,alp,bet,opts);
%
% W: W matrix (N*KM)
...
% alp: Huber alpha parameter.
% bet: (beta) -- ratio of Huber prior strength to image noise precision.
...
if (GRADTEST)
    [D,G] = local_gradcheck(x,W,Y,alp,bet,biv,bih);
...
if (nargout <4)
    [X,opts,flog] = scg(@local_func,x,opts,@local_grad,W,Y,alp,bet,biv,bih);
else
    [X,opts,flog,plog,sclog] = scg(@local_func,x,opts,@local_grad,W,Y,alp,bet,biv,bih);
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function e = local_func(x,W,Y,alp,bet,biv,bih)
e = Y'-x*W;
e = sum(e.^2) + elle_eval_huber(x,alp,bet,biv,bih);
return
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function g = local_grad(x,W,Y,alp,bet,biv,bih)
r = Y'-x*W;
g = W*r';
g = -2.*g' + elle_eval_huber_grad(x,alp,bet,biv,bih);
return
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
