...
int mycompare(const void * a, const void * b) {
  return (int) (*(sort_data+ *((unsigned int *)a)) -
                *(sort_data+ *((unsigned int *)b)));
}
...
{ 
  ...
  int nzmax, nrow, ncol, len, ii, jj, ka, kb, icol, ipos, k,
      *jw, sort_size, max_sort_size, i;
  unsigned int *sorti;    
    
  if ((nrhs < 2)|(nlhs!=1)) {
    PRINTF("mex_amub.cpp: bad args\nusage: mex_mymult "
           "(sparse matrix multiplication) "
           "usage: real(A) * real(B) = mex_amub(A,B,nnz);\n");
    return;
  }
  ...
  nzmax = 10;
  if (nrhs == 3) {
    nzmax = ((int) *mxGetPr(prhs[2])+10) > nzmax ?
            ((int) *mxGetPr(prhs[2])+10) : nzmax;
  }
  nrow = mxGetM(A);
  ncol = mxGetN(B);
