function [xi,yi]=gdsolver(fx,dfx)
...
Gx=@(x,fx) (cellfun(@(c) c(x),fx))';      % nonlinear system
% eval by calling `Gx(x0,fx)`
Fx=@(x,Gx,fx) .5*Gx(x,fx).*Gx(x,fx);      % objective function
% eval by calling `Fx(x0,Gx,fx)`
J_G=@(x,dfx) cell2mat(cellfun(@(c) (c(x))',dfx,'uniformoutput',false));
% eval by calling `J_G(x0,dfx)`
dFx=@(x,J_G,dfx,Gx,fx) (J_G(x,dfx))'*Gx(x,fx); % derivative
% eval by calling `dFx(x0,J_G,dfx,Gx,fx)`
