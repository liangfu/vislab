...
PI=1./(1+exp(-PRIOR.f));
W_test=diag(PI.*(1-PI));
K=kern(PRIOR.X,PRIOR.X); 
L_test=chol(eye(size(K,1))+sqrt(W_test)*K*sqrt(W_test));
fs=kern(PRIOR.X,X_test)'*((PRIOR.Y+1)/2-PI);
% v=L_test\(sqrt(K)*kern(PRIOR.X,X_test));
% Vfs=kern(X_test,X_test)-v'*v;
