...
% 
% N1=100;N2=5;D=3;
% X = [randn(N1,D); randn(N2,D) + 1.5];  
% Y = [zeros(N1,1); ones(N2,1)];
% W = lda(X,Y);
% L = [ones(size(X,1),1) X] * W';
% P = exp(L) ./ repmat(sum(exp(L),2),[1 2]);
% sum(P(1:N1,1)>0.5)/double(N1),(N2-sum(P(N1+1:N1+N2,1)>0.5))/double(N2)
% plot(X(1:N1,1),X(1:N1,2),'r.',X(N1+1:N1+N2,1),X(1+N1:N1+N2,2),'b.')


function W = lda_new(Input,Target,Priors)
...
PooledCov=PooledCov+eye(size(PooledCov)).*1e-5;
...
L = [ones(size(X,1),1) X] * W';
P = exp(L) ./ repmat(sum(exp(L),2),[1 2]);
sum(find(P(Y==C(1),1)>0.5))/double(sum(find(Y==C(1))))
sum(find(P(Y==C(2),1)>0.5))/double(sum(find(Y==C(2))))
