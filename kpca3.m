...
% use all patterns to compute the covariance matrix 
% carry out Kernel PCA 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
for i=1:cov_size, 
for j=i:cov_size, 
% K(i,j) = exp(-norm(patterns(i,:)-patterns(j,:))^2/rbf_var); 
K(i,j) = exp(-sum(sum((patterns(i,:)-patterns(j,:)).^2))/rbf_var); 
K(j,i) = K(i,j);
end 
end 
...
for i=1:cov_size, evecs(:,i) = evecs(:,i)/(sqrt(evals(i))); end 
